...
export async function searchProducts(params: SearchParams) {
    ...
    const res = await fetch(`${API_BASE_URL}/api/products/search?${qs.toString()}`, {
         credentials: "include"
    });
...
export async function getHighlightedProducts(maxItems: number = 3) {
    const res = await fetch(`${API_BASE_URL}/api/products/highlighted/${maxItems}`, {
        method: "GET",
        credentials: "include"
    });
...
export async function getProductsByCategory(categoryId: string) {
    const res = await fetch(`${API_BASE_URL}/api/products/by-category/${categoryId}`, {
         method: "GET",
         credentials: "include"
    });
